...
class Settlement:
    ...
    cities = dict()            # словарь всех городов мира


    def __init__(self,state, coordinates, biomMap, name, serfs,schoolers, facnum = 0, size=1):
        self.size = size           # размер города. город будет расти на карте мира. алгоритм уже придумал. руки просто не дошли написать
        biomMap[coordinates] = 2        # переписываем в координатах города биом основной карты, чтобы сменить биом на городской (и отрисовать это)
        ...
        Settlement.Check_surroundings(self,(self.area[0][0],self.area[0][1]),biomMap,self.possible_area)
        Settlement.Check_surroundings_city(self, (self.area[0][0], self.area[0][1]), biomMap, self.possible_grain_fields)
        self.name = name
        ...
        # Плохо в конструктор засовывать обработчики TODO
        ...
        Settlement.cities[self.name] = self
        self.population = 0
        self.city = False
        self.state = state                         # принадлежность государству
        # TODO констуктор меняет входящие поля
    ...
    def Check_surroundings(self,coordinates,biomMap,possible_list):
        if biomMap[coordinates[0] - 1, coordinates[1]] != 0 and biomMap[coordinates[0] - 1, coordinates[1]] != 2 and biomMap[coordinates[0] - 1, coordinates[1]] != 5:
            if (coordinates[0] - 1, coordinates[1]) not in possible_list:
                possible_list.append((coordinates[0] - 1, coordinates[1]))

        if biomMap[coordinates[0] + 1, coordinates[1]] != 0 and biomMap[
            coordinates[0] + 1, coordinates[1]] != 2 and biomMap[
            coordinates[0] + 1, coordinates[1]] != 5:
            if (coordinates[0] + 1, coordinates[1]) not in possible_list:
                possible_list.append((coordinates[0] + 1, coordinates[1]))

        if biomMap[coordinates[0], coordinates[1] - 1] != 0 and biomMap[
            coordinates[0], coordinates[1] - 1] != 2 and biomMap[
    ...
    def Check_surroundings_city(self,coordinates,biomMap,possible_list):
        if biomMap[coordinates[0] - 1, coordinates[1]] != 0 and biomMap[coordinates[0] - 1, coordinates[1]] != 2:
            if (coordinates[0] - 1, coordinates[1]) not in possible_list:
                possible_list.append((coordinates[0] - 1, coordinates[1]))
    ...
    def Check_to_delete(self,coordinates,biomMap):
        """
    ...
    def City_growth(self,biomMap,background,gm):
        if self.population > 5000*self.size:
            ...
            for i in range(len(self.area)):
                if biomMap[self.area[i][0]-1,self.area[i][1]] != 0 and biomMap[self.area[i][0]-1,self.area[i][1]] != 2:           # и надо будет позже ещё учесть горы и иные биомы
                    newlands.append((self.area[i][0]-1,self.area[i][1]))

                if biomMap[self.area[i][0]+1,self.area[i][1]] != 0 and biomMap[self.area[i][0]+1,self.area[i][1]] != 2:
                    newlands.append((self.area[i][0]+1,self.area[i][1]))

                if biomMap[self.area[i][0],self.area[i][1]-1] != 0 and biomMap[self.area[i][0],self.area[i][1]-1] != 2:
            ...
            if self.possible_area:
                ...
                if biomMap[randchoice[0],randchoice[1]] == 5 and [randchoice[0],randchoice[1]] in self.grain_fields:
                    index123 = self.grain_fields.index([randchoice[0],randchoice[1]])
                    del self.grain_fields[index123]

                biomMap[randchoice[0],randchoice[1]] = 2
                self.area.append([randchoice[0],randchoice[1]])
                background.set_at((randchoice[0],randchoice[1]), (50, 50, 50))
                Settlement.check_surroundings_city(self, randchoice, biomMap, self.possible_area)
                    ...
                    if len(self.grain_fields) < i.num_workers*self.state.serf_land_right_per_capita:# тут надо очень внимательно смотреть на коэффициенты. переделать скорее всего
                        for diff in range(int(round(i.num_workers*self.state.serf_land_right_per_capita - len(self.grain_fields) + 0.5))): # округляем в бОльшую сторону
                            if self.possible_grain_fields:
                                ...
                                biomMap[resource_coord] = 5
                                self.grain_fields.append(resource_coord)
                                Settlement.check_surroundings(self,resource_coord,biomMap,self.possible_grain_fields)
